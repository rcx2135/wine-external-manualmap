...
typedef BOOL      (WINAPI *f_DllMain_t)(void *handle_dll, DWORD dwReason, void *pReserved);
...
__attribute__((noinline, section(".text.shellcode")))
void __stdcall shellcode(mapping_data_t *p_data) {
    if (!p_data)
        // error on purpose
{
    ...
    f_GetProcAddress_t  _GetProcAddress  = (f_GetProcAddress_t)p_data->get_proc_address;
    f_GetModuleHandleA_t _GetModuleHandleA = (f_GetModuleHandleA_t)p_data->get_module_handle_a;
#ifdef _WIN64
    f_RtlAddFunctionTable_t _RtlAddFunctionTable = (f_RtlAddFunctionTable_t)p_data->rtl_add_function_table;
#endif
            ...
            while (reloc < p_reloc_end && reloc->SizeOfBlock) {
                ...
                for (DWORD i = 0; i < count; i++, p_info++) {
                    if (RELOC_FLAG(*p_info)) {
                        ULONG_PTR *pPatch =
                            (ULONG_PTR *)(p_base + reloc->VirtualAddress + (*p_info & 0xFFF));
                        *pPatch += delta;
                    }
                }
    ...
    if (import_dir->Size) {
        ...
        while (p_import->Name) {
            ...
            HINSTANCE   handle_dll     = _GetModuleHandleA ? (HINSTANCE)_GetModuleHandleA(modName) : NULL;
            if (!handle_dll)
                handle_dll = _LoadLibraryA(modName);
            ULONG_PTR  *p_thunk_ref = (ULONG_PTR *)(p_base + p_import->OriginalFirstThunk);
